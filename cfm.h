...
#define CFM_H
    ...
    static inline char* getListedDirectories(char* directory){
        char path[1024];
    ...
    static inline void* findFile(char* name, char* root, char* type){
        if(!type) return NULL;
        if(
            strcmp(type, "fr") == 0 && 
            strcmp(type, "frb") == 0 &&
    ...
    static inline char* ParseFile(FILE* fp, size_t* line){
    ...
    static inline int dirExists(const char* path) {
        struct stat sb;
    ...
    static inline TCHAR* getCurrentDirectory(void){
        ...
        if (dwRet == 0) {
            ...
            // In a real application, you'd reallocate a larger buffer and call  again
            return NULL;
        }
        return &tszBuffer;
    }
